...
export default function StatsCards({ invoices }: Props) {
  ...
  const Card = ({ title, value, icon: Icon, bg, iconBg }: {
    ...
    <div className={`${bg} rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow border border-gray-100`}>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <Card title="총 청구서" value={`${totalCount}건`} icon={DocumentTextIcon} bg="bg-indigo-100" iconBg="bg-indigo-500" />
      <Card title="결제완료" value={`${paidCount}건`} icon={CheckCircleIcon} bg="bg-green-100" iconBg="bg-green-500" />
      <Card title="미결제" value={`${unpaidCount}건`} icon={ClockIcon} bg="bg-orange-100" iconBg="bg-orange-500" />
      <Card title="총 청구 금액" value={`₩${totalAmount.toLocaleString()}원`} icon={CurrencyDollarIcon} bg="bg-blue-100" iconBg="bg-blue-500" />
    </div>
  );
}
