...
export function buildWorkbook(snapshot: AppSnapshot): ExcelJS.Workbook {
  const companyInfo = (snapshot.companyInfo ?? {}) as Record<string, unknown>;
...
async function writeViaElectron(ab: ArrayBuffer): Promise<boolean> {
  ...
  if (api == null || typeof api.writeXlsx !== 'function') return false;
  try {
...
export function scheduleMirror(snapshot: AppSnapshot, delayMs: number = 1 * MS_IN_SECOND): void {
