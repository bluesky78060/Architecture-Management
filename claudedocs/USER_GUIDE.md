...
1. 화면 하단의 **"0원"** 버튼(총합계 표시 버튼)을 클릭합니다
2. 또는 **"추가"** 버튼을 클릭합니다
...
```
Excel에서 한 번에 데이터를 입력하고
"일괄 추가"로 한 번에 업로드하면 됩니다.
