  ...
  return (
    <div className="bg-white rounded-md shadow-sm p-3 mb-4">
      <div className="flex items-center space-x-3 flex-wrap">
        ...
        <div className="relative inline-flex items-center">
          <select value={selectedStatus} onChange={(e) => onChangeStatus(e.target.value)} disabled={!selectedClient} className={`border border-gray-300 rounded-md px-3 pr-8 py-1.5 bg-white text-xs hover:bg-gray-50 focus:outline-none focus:ring-1 focus:ring-indigo-500 appearance-none cursor-pointer min-w-0 ${!selectedClient ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700'}`}>
            <option value="">{!selectedClient ? '건축주 선택 후' : '전체 상태'}</option>
